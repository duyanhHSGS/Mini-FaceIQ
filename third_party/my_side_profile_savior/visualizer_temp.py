...
def _draw_original(axis, record, show_auxiliary: bool) -> None:
...
def _draw_legacy_3ddfa(axis, record, result: dict) -> None:
    with Image.open(record.image_path) as source:
        image = source.convert("RGB")
...
class InteractiveLandmarkViewer:
    ...
    def __init__(
        ...
        *,
        ...
        self.instruction_text = self.figure.text(
            ...
            0.165,
            (
                "1) Click a numbered Multi-PIE dot on the LEFT.  "
                "2) Click its named legacy landmark on the RIGHT.  "
                "3) Assign, NONE, or ?.  4) SAVE."
            ),
            fontsize=10,
            weight="bold",
        )
    ...
    def _render(self) -> None:
        ...
        self.dataset_axis.set_title(
            f"CLICK DATASET DOT 0–38\n{identity}",
            fontsize=10,
            weight="bold",
        )
        self.legacy_axis.set_title(
            "CLICK NAMED LEGACY LANDMARK\n"
...
def _build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Open the clickable landmark-mapping UI instead of writing a "
