...
def analyze_face(path, mat_path=None, cb=None):
    ...
    if mat_path and os.path.exists(mat_path):
        mr=np.load(mat_path); mp_=mat_process(mr).transpose(1,2,0)
...
def make_heatmap(img224,masks,deltas):
    ...
    cv2.putText(comb,"XAU",(5,ov.shape[0]+15),cv2.FONT_HERSHEY_SIMPLEX,.45,(255,255,255),1)
    cv2.putText(comb,"DEP",(170,ov.shape[0]+15),cv2.FONT_HERSHEY_SIMPLEX,.45,(255,255,255),1)
